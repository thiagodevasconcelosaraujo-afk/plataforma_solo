...
if botao_salvar:
    if not novo_ambiente or not nova_parcela:
        ...
        try:
            ...
            st.sidebar.error("Erro: Preencha apenas números nos campos!")
...
if botao_deletar:
    if deletar_ambiente == "" or not deletar_parcela:
        st.sidebar.error("Selecione o Ambiente e digite o ID!")
    else:
...
with aba_geral:
    ...
    st.download_button(label="📥 Baixar Banco de Dados Completo (.Excel)", data=dados_excel, file_name="banco_solo.xlsx")
...
with aba_comparativo:
    ...
    atributos_selecionados = st.multiselect(
        "Selecione as propriedades do solo para comparar simultaneamente:",
    ...
    if not atributos_selecionados:
        ...
        # LINHAS ULTRA CURTAS: Evita o erro de quebra de parênteses do editor Nano do Linux
        fig_colunas = px.bar(
            df_melt,
